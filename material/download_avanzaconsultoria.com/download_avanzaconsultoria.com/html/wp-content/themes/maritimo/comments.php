	...
	if ('comments.php' == basename($_SERVER['SCRIPT_FILENAME']))
        ...
        if (!empty($post->post_password)) { // if there's a password
            if ($_COOKIE['wp-postpass_' . COOKIEHASH] != $post->post_password) {  // and it doesn't match the cookie
				...
				<p class="nocomments"><?php _e("Este articulo esta protegido con clave. Introduce tu clave para ver los comentarios."); ?><p>
...
<?php if ($comments) : ?>
	<h3 id="comments"><?php comments_number('No hay comentarios', 'Un comentario', '% comentarios' );?> en &#8220;<?php the_title(); ?>&#8221;</h3> 
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li id="comment-<?php comment_ID() ?>" class="<?php echo $oddcomment; /* Style differently if comment author is blog author */ if ($comment->comment_author_email == get_the_author_email()) { echo ' authorcomment'; } ?>">
			<div class="cmtinfo"><small class="commentmetadata"><a href="#comment-<?php comment_ID() ?>" title="">#</a> </small><cite><?php comment_author_link() ?></cite><em>el <?php comment_date('d M Y') ?> a las <?php comment_time() ?> <?php edit_comment_link('editar esto','',''); ?></em></div>
...
<?php if ($post-> comment_status == "open") {?>
	<?php comments_rss_link('Comentarios RSS'); ?>
<?php }; ?>
</p>
</div>
...
<p>Tienes que estar <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php the_permalink(); ?>">registrado</a> para escribir un comentario.</p>
<?php else : ?>
...
<label for="email"><small>E-Mail (no sera visible) <?php if ($req) _e('(requerido'); ?></small></label></p>
...
<!--<p><small><strong>XHTML:</strong> Puedes utiliza estas etiquetas: <?php echo allowed_tags(); ?></small></p>-->
