<?php // Do not delete these lines
	...
	if (!empty($post->post_password)) { // if there's a password
		if ($_COOKIE['wp-postpass_' . COOKIEHASH] != $post->post_password) {  // and it doesn't match the cookie
			...
			<p class="nocomments">Post protegido con contraseña, ingresela para ver el post.</p>
...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li class="<?php echo $oddcomment; ?> <?php if(function_exists("author_highlight")) author_highlight(); ?>" id="comment-<?php comment_ID() ?>">
			...
			<em>Tu comentario está esperando ser moderado.</em>
			<?php endif; ?>
			<br />

			<small class="commentmetadata"><a href="#comment-<?php comment_ID() ?>" title=""><?php comment_date('F jS, Y') ?> el <?php comment_time() ?></a> <?php edit_comment_link('e','',''); ?></small>
...
<h3 id="comments">Deje un comentario</h3>

<?php if ( get_option('comment_registration') && !$user_ID ) : ?>
<p class="alert">Debes estar <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php the_permalink(); ?>">registrado</a> para dejar un comentario.</p>
<?php else : ?>
