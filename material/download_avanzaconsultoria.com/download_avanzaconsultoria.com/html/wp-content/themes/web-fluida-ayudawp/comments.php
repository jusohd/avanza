	...
	if ('comments.php' == basename($_SERVER['SCRIPT_FILENAME']))
        ...
        if (!empty($post->post_password)) { // if there's a password
            if ($_COOKIE['wp-postpass_' . COOKIEHASH] != $post->post_password) {  // and it doesn't match the cookie
				...
				<p class="nocomments">Este post esta protegido por clave. Introduce la clave para ver los comentarios.<p>
...
<?php if ($comments) : ?>
	<h4 id="comments"><?php comments_number('No hay comentarios', 'Un comentario', '% comentarios' );?> a &#8220;<?php the_title(); ?>&#8221;</h4> 
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li class="<?php echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
			...
			<em>Tu comentario esta esperando aprobación.</em>
			<?php endif; ?>
			<br />
...
<h4 id="respond">Escribe un comentario</h4>
...
<label for="email"><small>Mail (no se hará público) <?php if ($req) echo "(requerido)"; ?></small></label></p>
