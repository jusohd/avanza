...
function get_breadcrumb() {
    ...
    echo 'Estás aquí : <a href="/" title="">Principal</a>';
...
function first_year() {
    ...
    $sql = "SELECT YEAR(post_date_gmt) FROM $wpdb->posts ORDER BY post_date_gmt ASC LIMIT 1";
    $year = $wpdb->get_var($sql);

    if ($year < $thisyear && $year != '0')
        echo $year.' - '.$thisyear;
    else
        echo $thisyear;
}
...
function get_newcalendar($daylength = 1) {
	...
	if ( $previous ) {
		...
		get_month_link($previous->year, $previous->month) . '" title="' . sprintf(__('Ver articulos de %1$s %2$s'), $month[zeroise($previous->month, 2)],
