				...
				<?php
					...
					if (!$send) { ?>
						...
						<div class="entry">
							...
							<p class="posted">* - Campors requeridos</p>
							<?php echo $error_msg;?>
							<form method="post" action="<?php echo "http://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; ?>" id="contactform">
								...
								<fieldset>
									...
									<strong>Prueba que no eres un robot:<?php echo $cp_question; ?> </strong>*<br/>
									<input type="text" class="textbox" id="cp_ans" name="cp_ans" value="<?php echo $cp_ans ;?>" />
									<p class="posted">[Just to prove you are a person and not a spam script]</p><br/>
					...
					<?php
						...
						if (@mail(get_settings('admin_email'), $email_subject, $email_text, $header)) {
							echo "<h2>Hey " . $displayName . ",</h2><p>Gracias por dejar tu mensaje, te responderemos a la brevedad!.</p>";
						}
					}
					?>
				<?php endwhile; ?>
