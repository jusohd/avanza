		...
		if ($comments) {
			foreach ($comments as $comment) {
				...
				if ($comment_style == 1) {
					...
					$output .= '" title="Ver el comentario completo por ' . $comment_author.'">' . $comment_excerpt.'</a>' . $after;
				}
			}
			$output = convert_smilies($output);
		} else {
			$output .= $before . "Nada encontrado" . $after;
		}
	    echo $output;
	}
}
...
function wp_pagenavi($before = '', $after = '', $prelabel = '', $nxtlabel = '', $pages_to_show = 5, $always_show = false) {
	...
	if (!is_single()) {
		...
		if($max_page > 1 || $always_show) {
			echo "$before <div class='Nav'><span>P&aacute;ginas ($max_page): </span>";
			if ($paged >= ($pages_to_show-1)) {
				echo '<a href="'.get_pagenum_link().'">&laquo; Primero</a> ... ';
			}
			...
			if (($paged+$half_pages_to_show) < ($max_page)) {
				echo ' ... <a href="'.get_pagenum_link($max_page).'">&Uacute;ltima &raquo;</a>';
			}
			echo "</div> $after";
		}
	}
