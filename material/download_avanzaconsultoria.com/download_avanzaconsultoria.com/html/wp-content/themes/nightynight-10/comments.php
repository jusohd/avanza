<?php // Do not delete these lines
	...
	if (!empty($post->post_password)) { // if there's a password
		if ($_COOKIE['wp-postpass_' . COOKIEHASH] != $post->post_password) {  // and it doesn't match the cookie
			...
			<p class="nocomments">Esta entrada está protegida. Introduce tu clave para ver las críticas.<p>
...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		<div class="comm_panel">
			<b><?php comment_author_link() ?></b> el <?php comment_date('j F, Y') ?> a las <?php comment_time() ?> #
		</div>
		<div class="comm_text">
			<?php if ($comment->comment_approved == '0') : ?>
				Tu crítica está pendiente de aprobación.
			<?php endif; ?>
			<?php comment_text() ?>
		</div>
		<br />
 ...
 <?php else : // this is displayed if there are no comments so far ?>
	...
	<?php if ('open' == $post->comment_status) : ?>
	 ...
	 <?php else : // comments are closed ?>
		...
		<p class="nocomments">Las críticas están cerradas.</p>
...
<a id="respond"></a>
	...
	<p>Debes <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php the_permalink(); ?>">registrarte</a> para publicar una crítica.</p>
	<?php else : ?>
